require 'Items/ProceduralDistributions'

-- Autotsar.AtTuningMagJeep

--- VANILLA MechanicMag3:
-- BookstoreMisc 2
-- CrateMagazines 1
-- CrateMechanics 1
-- GasStorageMechanics 1
-- LibraryBooks 1
-- LivingRoomShelf 0.1
-- LivingRoomShelfNoTapes 0.1
-- LivingRoomSideTable 0.1
-- LivingRoomSideTableNoRemote 0.1
-- MagazineRackMixed 1
-- MechanicShelfBooks 2
-- MechanicSpecial 1
-- PostOfficeMagazines 1
-- ShelfGeneric 0.1
-- StoreShelfMechanics 1
-- ToolStoreBooks 2

--VANILLA:
table.insert(ProceduralDistributions.list["BookstoreMisc"].items, "Autotsar.AtTuningMagJeep");
table.insert(ProceduralDistributions.list["BookstoreMisc"].items, 0.5);
table.insert(ProceduralDistributions.list["CrateMagazines"].items, "Autotsar.AtTuningMagJeep");
table.insert(ProceduralDistributions.list["CrateMagazines"].items, 0.25);
table.insert(ProceduralDistributions.list["CrateMechanics"].items, "Autotsar.AtTuningMagJeep");
table.insert(ProceduralDistributions.list["CrateMechanics"].items, 0.25);
table.insert(ProceduralDistributions.list["GasStorageMechanics"].items, "Autotsar.AtTuningMagJeep");
table.insert(ProceduralDistributions.list["GasStorageMechanics"].items, 0.25);
table.insert(ProceduralDistributions.list["LibraryBooks"].items, "Autotsar.AtTuningMagJeep");
table.insert(ProceduralDistributions.list["LibraryBooks"].items, 0.25);
table.insert(ProceduralDistributions.list["LivingRoomShelf"].items, "Autotsar.AtTuningMagJeep");
table.insert(ProceduralDistributions.list["LivingRoomShelf"].items, 0.03);
table.insert(ProceduralDistributions.list["LivingRoomShelfNoTapes"].items, "Autotsar.AtTuningMagJeep");
table.insert(ProceduralDistributions.list["LivingRoomShelfNoTapes"].items, 0.03);
table.insert(ProceduralDistributions.list["LivingRoomSideTable"].items, "Autotsar.AtTuningMagJeep");
table.insert(ProceduralDistributions.list["LivingRoomSideTable"].items, 0.03);
table.insert(ProceduralDistributions.list["LivingRoomSideTableNoRemote"].items, "Autotsar.AtTuningMagJeep");
table.insert(ProceduralDistributions.list["LivingRoomSideTableNoRemote"].items, 0.03);
table.insert(ProceduralDistributions.list["MagazineRackMixed"].items, "Autotsar.AtTuningMagJeep");
table.insert(ProceduralDistributions.list["MagazineRackMixed"].items, 0.25);
table.insert(ProceduralDistributions.list["MechanicShelfBooks"].items, "Autotsar.AtTuningMagJeep");
table.insert(ProceduralDistributions.list["MechanicShelfBooks"].items, 0.5);
table.insert(ProceduralDistributions.list["MechanicSpecial"].items, "Autotsar.AtTuningMagJeep");
table.insert(ProceduralDistributions.list["MechanicSpecial"].items, 0.25);
table.insert(ProceduralDistributions.list["PostOfficeMagazines"].items, "Autotsar.AtTuningMagJeep");
table.insert(ProceduralDistributions.list["PostOfficeMagazines"].items, 0.25);
table.insert(ProceduralDistributions.list["ShelfGeneric"].items, "Autotsar.AtTuningMagJeep");
table.insert(ProceduralDistributions.list["ShelfGeneric"].items, 0.1);
table.insert(ProceduralDistributions.list["StoreShelfMechanics"].items, "Autotsar.AtTuningMagJeep");
table.insert(ProceduralDistributions.list["StoreShelfMechanics"].items, 0.25);
table.insert(ProceduralDistributions.list["ToolStoreBooks"].items, "Autotsar.AtTuningMagJeep");
table.insert(ProceduralDistributions.list["ToolStoreBooks"].items, 1);
--OWN:
table.insert(ProceduralDistributions.list["BookstoreBooks"].items, "Autotsar.AtTuningMagJeep");
table.insert(ProceduralDistributions.list["BookstoreBooks"].items, 1);

-- ATAJeepRoofTentItem
table.insert(ProceduralDistributions.list["CrateMechanics"].items, "Autotsar.ATAJeepRoofTentItem");
table.insert(ProceduralDistributions.list["CrateMechanics"].items, 0.5);
table.insert(ProceduralDistributions.list["GarageMechanics"].items, "Autotsar.ATAJeepRoofTentItem");
table.insert(ProceduralDistributions.list["GarageMechanics"].items, 0.5);
table.insert(ProceduralDistributions.list["MechanicShelfTools"].items, "Autotsar.ATAJeepRoofTentItem");
table.insert(ProceduralDistributions.list["MechanicShelfTools"].items, 0.5);
table.insert(ProceduralDistributions.list["MechanicShelfTools"].junk.items, "Autotsar.ATAJeepRoofTentItem");
table.insert(ProceduralDistributions.list["MechanicShelfTools"].junk.items, 0.5);
table.insert(ProceduralDistributions.list["MechanicShelfWheels"].items, "Autotsar.ATAJeepRoofTentItem");
table.insert(ProceduralDistributions.list["MechanicShelfWheels"].items, 0.5);

-- ATAJeepSnorkelItem
table.insert(ProceduralDistributions.list["CrateMechanics"].items, "Autotsar.ATAJeepSnorkelItem");
table.insert(ProceduralDistributions.list["CrateMechanics"].items, 0.25);
table.insert(ProceduralDistributions.list["GarageMechanics"].items, "Autotsar.ATAJeepSnorkelItem");
table.insert(ProceduralDistributions.list["GarageMechanics"].items, 0.25);
table.insert(ProceduralDistributions.list["MechanicShelfTools"].items, "Autotsar.ATAJeepSnorkelItem");
table.insert(ProceduralDistributions.list["MechanicShelfTools"].items, 0.25);
table.insert(ProceduralDistributions.list["MechanicShelfTools"].junk.items, "Autotsar.ATAJeepSnorkelItem");
table.insert(ProceduralDistributions.list["MechanicShelfTools"].junk.items, 0.25);
table.insert(ProceduralDistributions.list["MechanicShelfWheels"].items, "Autotsar.ATAJeepSnorkelItem");
table.insert(ProceduralDistributions.list["MechanicShelfWheels"].items, 0.25);


-- ATAJeepBumper2Item
table.insert(ProceduralDistributions.list["CrateMechanics"].items, "Autotsar.ATAJeepBumper2Item");
table.insert(ProceduralDistributions.list["CrateMechanics"].items, 1);
table.insert(ProceduralDistributions.list["GarageMechanics"].items, "Autotsar.ATAJeepBumper2Item");
table.insert(ProceduralDistributions.list["GarageMechanics"].items, 1);
table.insert(ProceduralDistributions.list["MechanicShelfTools"].items, "Autotsar.ATAJeepBumper2Item");
table.insert(ProceduralDistributions.list["MechanicShelfTools"].items, 1);
table.insert(ProceduralDistributions.list["MechanicShelfTools"].junk.items, "Autotsar.ATAJeepBumper2Item");
table.insert(ProceduralDistributions.list["MechanicShelfTools"].junk.items, 1);
table.insert(ProceduralDistributions.list["MechanicShelfWheels"].items, "Autotsar.ATAJeepBumper2Item");
table.insert(ProceduralDistributions.list["MechanicShelfWheels"].items, 1);


-- ATAJeepBumper3Item
table.insert(ProceduralDistributions.list["CrateMechanics"].items, "Autotsar.ATAJeepBumper3Item");
table.insert(ProceduralDistributions.list["CrateMechanics"].items, 0.5);
table.insert(ProceduralDistributions.list["GarageMechanics"].items, "Autotsar.ATAJeepBumper3Item");
table.insert(ProceduralDistributions.list["GarageMechanics"].items, 0.5);
table.insert(ProceduralDistributions.list["MechanicShelfTools"].items, "Autotsar.ATAJeepBumper3Item");
table.insert(ProceduralDistributions.list["MechanicShelfTools"].items, 0.5);
table.insert(ProceduralDistributions.list["MechanicShelfTools"].junk.items, "Autotsar.ATAJeepBumper3Item");
table.insert(ProceduralDistributions.list["MechanicShelfTools"].junk.items, 0.5);
table.insert(ProceduralDistributions.list["MechanicShelfWheels"].items, "Autotsar.ATAJeepBumper3Item");
table.insert(ProceduralDistributions.list["MechanicShelfWheels"].items, 0.5);